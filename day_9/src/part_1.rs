...
fn give_estimate(mut container: Vec<Vec<i64>>) -> i64{
    for x in (1..container.len()-1).rev(){
        let new_value =  container.get(x-1).expect("no container").last().expect("no num") + container.get(x).expect("no container").last().expect("no num");
        
        container.get_mut(x-1).expect("no container").push(new_value);
    ...
    return container.get(0).expect("no container").last().expect("no num").clone()
}
