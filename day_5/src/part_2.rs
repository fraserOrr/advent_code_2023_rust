...
fn main()  {
    let file = fs::read_to_string("./src/input2.txt").expect("Should have been able to read the file");
    ...
    let mut lowest: i64 = 99999999;
    for x in (0..seeds.len()-2).step_by(2){
        for i in seeds[x]..(seeds[x]+seeds[x+1]){
            //println!("{i}");
        ...
        let cur_location = calcseed(&mut tmp_seed,&seed_2_soil,&soil_2_fert,&fert_2_water, &water_2_light,&light_2_temp, &temp_2_hum, &hum_2_loc );

        if cur_location < lowest{
            lowest = cur_location;
        };
        }
...
fn calcseed(seed: &mut Seed,seed_2_soil: &Vec<Mapping>,soil_2_fert: &Vec<Mapping>,fert_2_water: &Vec<Mapping>, water_2_light: &Vec<Mapping>,light_2_temp: &Vec<Mapping>, temp_2_hum: &Vec<Mapping>, hum_2_loc: &Vec<Mapping> ) -> i64 {
